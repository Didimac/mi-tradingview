/**
 * Two-Phase Scorer Backtest — OPTIMIZED v2
 * Cambios: score 82, ATR SL 1.5/TP1 2.25/TP2 4.5, max 1 pos, filtro tendencia EMA55
 * Usage: npx tsx scripts/run-two-phase-optimized.ts
 */
...
const CFG_OPTIMIZED: Config = {
  label: "OPTIMIZADO",
  scoreThreshold: 82,         // CAMBIO 1: 70 → 82
  scoreCancelThreshold: 60,
  riskPerTrade: 0.015,
  maxOpenPositions: 1,         // CAMBIO 3: 3 → 1
  expiryBars: 2,
  timeoutBars: 48,
  idealZoneTolerance: 0.001,
  cooldownBars: 2,
  atrSlMult: 1.5,             // CAMBIO 2: 1.8 → 1.5
  atrTp1Mult: 2.25,           // CAMBIO 2: 1.8 → 2.25
  atrTp2Mult: 4.5,            // CAMBIO 2: 3.6 → 4.5
  useTrendFilter: true,        // CAMBIO 4: filtro EMA55
};
...
function runBacktestWithConfig(
  ...
  for (let i = minStart; i < totalBars; i++) {
    ...
    for (const sym of PAIRS) {
      ...
      // CAMBIO 4: Filtro de tendencia obligatorio
      if (cfg.useTrendFilter) {
        const ema55v = ind.ema55[i];
        if (isNaN(ema55v)) continue;
        if (bs.direction === "LONG" && price <= ema55v) {
          st.filteredByTrend++;
          continue;
        }
        if (bs.direction === "SHORT" && price >= ema55v) {
          st.filteredByTrend++;
          continue;
        }
      }
...
async function main() {
  console.log("═══════════════════════════════════════════════════════════════════════");
  console.log("  TWO-PHASE SCORER — BACKTEST COMPARATIVO (Original vs Optimizado)");
  console.log("  10 pares × 2 anos × 1H — Capital inicial: $1,000");
  console.log("═══════════════════════════════════════════════════════════════════════\n");

  console.log("  CAMBIOS APLICADOS:");
  console.log("  [1] Score threshold:  70 → 82");
  console.log("  [2] ATR SL: 1.8→1.5 | TP1: 1.8→2.25 | TP2: 3.6→4.5");
  console.log("  [3] Max posiciones:   3 → 1");
  console.log("  [4] Filtro tendencia: precio vs EMA55 obligatorio\n");
  ...
  // Run both configs
  console.log("\n  Ejecutando backtest ORIGINAL...");
  const resOrig = runBacktestWithConfig(CFG_ORIGINAL, allCandles, allIndicators);

  console.log("  Ejecutando backtest OPTIMIZADO...");
  const resOpt = runBacktestWithConfig(CFG_OPTIMIZED, allCandles, allIndicators);

  // ─── Print per-pair results for OPTIMIZED ───

  console.log("\n═════════════════════════════════════════════════════════════════════════════════════════════════════════════════");
  console.log("  RESULTADOS POR PAR — OPTIMIZADO (score≥82, SL 1.5x, TP1 2.25x, TP2 4.5x, max 1 pos, filtro EMA55)");
  console.log("═════════════════════════════════════════════════════════════════════════════════════════════════════════════════");
  ...
  for (const sym of PAIRS) {
    const st = resOpt.pairStats.get(sym)!;
  ...
  console.log("\n═══════════════════════════════════════════════════════════════════════");
  console.log("  TABLA COMPARATIVA: ANTES vs DESPUES");
  console.log("═══════════════════════════════════════════════════════════════════════\n");
  ...
  const rows: [string, string, string, string][] = [
    [
      "Senales F1",
      String(resOrig.totalF1),
      String(resOpt.totalF1),
      `${fmt(resOpt.totalF1 - resOrig.totalF1, 0)} (${((resOpt.totalF1 / resOrig.totalF1 - 1) * 100).toFixed(0)}%)`,
    ],
    [
      "Filtradas tendencia",
      "N/A",
      String(resOpt.totalFilteredByTrend),
      "NUEVO",
    ],
    [
      "Entradas F2",
      String(resOrig.totalF2),
      String(resOpt.totalF2),
      `${fmt(resOpt.totalF2 - resOrig.totalF2, 0)} (${((resOpt.totalF2 / resOrig.totalF2 - 1) * 100).toFixed(0)}%)`,
    ],
    [
      "% llego a zona",
      `${(resOrig.totalF2 / resOrig.totalF1 * 100).toFixed(1)}%`,
      `${(resOpt.totalF2 / resOpt.totalF1 * 100).toFixed(1)}%`,
      fmtPct((resOpt.totalF2 / resOpt.totalF1 - resOrig.totalF2 / resOrig.totalF1) * 100),
    ],
    [
      "Trades / semana",
      resOrig.tradesPerWeek.toFixed(1),
      resOpt.tradesPerWeek.toFixed(1),
      fmt(resOpt.tradesPerWeek - resOrig.tradesPerWeek),
    ],
    [
      "Win Rate global",
      `${resOrig.globalWR.toFixed(1)}%`,
      `${resOpt.globalWR.toFixed(1)}%`,
      fmtPct(resOpt.globalWR - resOrig.globalWR),
    ],
    [
      "Profit Factor",
      resOrig.globalPF === Infinity ? "inf" : resOrig.globalPF.toFixed(2),
      resOpt.globalPF === Infinity ? "inf" : resOpt.globalPF.toFixed(2),
      fmt(resOpt.globalPF - resOrig.globalPF),
    ],
    [
      "Retorno total",
      `${resOrig.totalReturn.toFixed(2)}%`,
      `${resOpt.totalReturn.toFixed(2)}%`,
      fmtPct(resOpt.totalReturn - resOrig.totalReturn),
    ],
    [
      "Capital final",
      `$${resOrig.finalCapital.toFixed(2)}`,
      `$${resOpt.finalCapital.toFixed(2)}`,
      `$${fmt(resOpt.finalCapital - resOrig.finalCapital)}`,
    ],
    [
      "Max Drawdown",
      `${resOrig.maxDD.toFixed(2)}%`,
      `${resOpt.maxDD.toFixed(2)}%`,
      fmtPct(resOpt.maxDD - resOrig.maxDD),
    ],
    [
      "Trades cerrados",
      String(resOrig.totalTradesClosed),
      String(resOpt.totalTradesClosed),
      fmt(resOpt.totalTradesClosed - resOrig.totalTradesClosed, 0),
    ],
    [
      "Mejor par",
      `${resOrig.bestPair} (+$${resOrig.bestRet.toFixed(0)})`,
      `${resOpt.bestPair} (+$${resOpt.bestRet.toFixed(0)})`,
      "",
    ],
    [
      "Peor par",
      `${resOrig.worstPair} ($${resOrig.worstRet.toFixed(0)})`,
      `${resOpt.worstPair} ($${resOpt.worstRet.toFixed(0)})`,
      "",
    ],
  ];
  ...
  const improved = resOpt.globalPF > resOrig.globalPF;
  const profitable = resOpt.totalReturn > 0;

  if (profitable) {
    console.log("  ✅ SISTEMA RENTABLE — El optimizado genera ganancias netas.");
  } else if (improved) {
    console.log("  ⚠️  MEJORA PARCIAL — PF mejorado pero aun no rentable. Ajustar mas.");
  } else {
    console.log("  ❌ SIN MEJORA — Se necesitan cambios mas profundos en la logica.");
  }

  if (resOpt.globalPF >= 1.3) {
    console.log("  🟢 PF >= 1.3: edge estadistico robusto.");
  } else if (resOpt.globalPF >= 1.0) {
    console.log("  🟡 PF entre 1.0-1.3: edge marginal, puede funcionar con mas filtros.");
  } else {
    console.log("  🔴 PF < 1.0: no hay edge. El sistema pierde dinero sistematicamente.");
  }

  console.log(`  Reduccion de overtrading: ${resOrig.tradesPerWeek.toFixed(1)} → ${resOpt.tradesPerWeek.toFixed(1)} trades/semana`);
  console.log(`  Senales filtradas por tendencia: ${resOpt.totalFilteredByTrend} (contra-tendencia eliminadas)`);
  console.log("═══════════════════════════════════════════════════════════════════════\n");
}
