/**
 * SmartScorer v2 Variante A — Final verification: BTC + SOL + BNB only
 * Usage: npx tsx scripts/run-smart-final.ts
 */
...
async function main(){
  console.log("═══════════════════════════════════════════════════════════════════════");
  console.log("  VERIFICACION FINAL — SmartScorer v2 Variante A");
  console.log("  Solo 3 pares robustos: BTC + SOL + BNB");
  console.log("  2 anos × 1H — Capital: $1,000");
  console.log("  Criterio: 3/3 pares PF > 1.0 en ambos anos → PRODUCCION");
  console.log("═══════════════════════════════════════════════════════════════════════\n");
  ...
  console.log("═══════════════════════════════════════════════════════════════════════\n");
  const phdr=["Periodo".padEnd(14),"Trades".padStart(7),"WR%".padStart(7),"PF".padStart(7),"Retorno%".padStart(10),"DD%".padStart(7),"Capital".padStart(10)].join(" | ");
  console.log(phdr);console.log("-".repeat(phdr.length));
  for(const[l,r]of [["Ano 1",p1.period],["Ano 2",p2.period],["2 anos",full.period]] as [string,PeriodResult][]){
    console.log([l.padEnd(14),String(r.trades).padStart(7),(r.wr.toFixed(1)+"%").padStart(7),(r.pf===Infinity?"inf":r.pf.toFixed(2)).padStart(7),((r.ret>=0?"+":"")+r.ret.toFixed(2)+"%").padStart(10),(r.dd.toFixed(1)+"%").padStart(7),("$"+r.capital.toFixed(0)).padStart(10)].join(" | "));
  }
  console.log("-".repeat(phdr.length));

  // Per pair × period
  console.log("\n═══════════════════════════════════════════════════════════════════════");
  console.log("  POR PAR × PERIODO");
  console.log("═══════════════════════════════════════════════════════════════════════\n");
  const dpHdr=["Par".padEnd(8),"Periodo".padEnd(8),"Trades".padStart(7),"TP1".padStart(5),"TP2".padStart(5),"SL".padStart(5),"WR%".padStart(7),"PF".padStart(7),"P&L".padStart(10),"PF>1?".padStart(6)].join(" | ");
  console.log(dpHdr);console.log("-".repeat(dpHdr.length));

  let allPass = true;
  for(const sym of PAIRS){
    for(const[label,run]of [["Ano 1",p1],["Ano 2",p2],["2 anos",full]] as [string,typeof p1][]){
      ...
      const pass=pf>1.0;
      if(label!=="2 anos"&&!pass) allPass=false;
      console.log([sym.replace("USDT","").padEnd(8),label.padEnd(8),String(s.f2).padStart(7),String(s.tp1).padStart(5),String(s.tp2).padStart(5),String(s.sl).padStart(5),(wr+"%").padStart(7),pfS.padStart(7),((pnl>=0?"+":"")+"$"+pnl.toFixed(2)).padStart(10),(pass?"✅":"❌").padStart(6)].join(" | "));
    }
  ...
  const p1pass=p1.period.pf>1.0;
  const p2pass=p2.period.pf>1.0;

  console.log(`  Ano 1 global PF > 1.0:  ${p1pass?"✅":"❌"} (${p1.period.pf.toFixed(2)})`);
  console.log(`  Ano 2 global PF > 1.0:  ${p2pass?"✅":"❌"} (${p2.period.pf.toFixed(2)})`);
  console.log(`  2 anos global PF > 1.1: ${full.period.pf>1.1?"✅":"❌"} (${full.period.pf.toFixed(2)})`);
  console.log(`  DD < 20%:               ${full.period.dd<20?"✅":"❌"} (${full.period.dd.toFixed(1)}%)`);
  console.log(`  Trades > 80:            ${full.period.trades>80?"✅":"❌"} (${full.period.trades})`);
  console.log(`  Todos pares PF>1 ambos: ${allPass?"✅":"❌"}`);

  if(p1pass&&p2pass&&full.period.pf>1.1&&full.period.dd<20){
    console.log("\n  🟢 APROBADO PARA PRODUCCION");
    console.log("  → Aplicar SmartScorer v2 Variante A con BTC+SOL+BNB");
  }else if(p1pass&&p2pass){
    console.log("\n  🟡 APROBADO CON RESERVAS — ambos periodos rentables pero check parcial");
    console.log("  → Puede aplicarse con monitoreo activo");
  }else{
    console.log("\n  🔴 NO APROBADO — inconsistencia entre periodos");
  }
  console.log("═══════════════════════════════════════════════════════════════════════\n");
}
